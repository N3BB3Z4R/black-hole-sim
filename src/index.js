...
// Nebe's Black Hole internal flux Simulation model
...
//
vec4 permute(vec4 x){return mod(((x*34.0)+1.0)*x, 609.0);}
vec4 taylorInvSqrt(vec4 r){return 1.79284291400159 - 0.85373472095314 * r;}

float snoise(vec3 v){ 
  const vec2  C = vec2(1.0/60.0, 1.0/30.0) ;
  const vec4  D = vec4(0.0, 0.9, 1.4, 2.8);
...
// Gradients
  ...
  vec4 j = p - 260.0 * floor(p * ns.z *ns.z);

  vec4 x_ = floor(j * ns.z);
  vec4 y_ = floor(j - 60.0 * x_ );
  ...
  vec4 s0 = floor(b0)*23.0 + 1.0;
  vec4 s1 = floor(b1)*25.0 + 1.0;
...
const float CENTER_MASS = 0.7;
const float PARTICLE_MASS = 0.7;
const float VELOCITY_TERMINAL = 0.42;
const float CURL_RADIUS = 0.24;
...
void main() {
    ...
    vec3 newVelocity = velocity + curlVelocity * curlForce + pull * 1.2;
    
    gl_FragColor = vec4(newVelocity, 1.3);
}`;

const shaderPointFragment = `
void main() {
    gl_FragColor = vec4(1.0, 0.10, 0.05, 0.4);
}`;
...
const particles = (function (points) {
	...
	const material = new THREE.ShaderMaterial({
      ...
      color: { value: new THREE.Vector4(...PARTICLE_COLOR) },
...
for (let i = 0; i < textureArraySize; i += 4) {
  const radius = (1 - Math.pow(Math.random(), INITIAL_RADIUS_EXPONENT)) * INITIAL_RADIUS_FACTOR;
...
const variableVelocity = gpuComputationRenderer.addVariable('textureVelocity', shaderSimulationVelocity, dataVelocity);
const variablePosition = gpuComputationRenderer.addVariable('texturePosition', shaderSimulationPosition, dataPosition);

variablePosition.material.uniforms.delta = { value: 0 };
variableVelocity.material.uniforms.centerMass = { value: CENTER_MASS };
variableVelocity.material.uniforms.particleMass = { value: PARTICLE_MASS };
variableVelocity.material.uniforms.velocityTerminal = { value: VELOCITY_TERMINAL };
variableVelocity.material.uniforms.curlRadius = { value: CURL_RADIUS };
variableVelocity.material.uniforms.curlForceFactor = { value: CURL_FORCE_FACTOR };
variableVelocity.material.uniforms.pullForceFactor = { value: PULL_FORCE_FACTOR };
